...
def draw_category_heatmap(fail_df):
    ...
    ax.set_title('카테고리 × Fail_Type 히트맵 (영역 불일치 발견)',
    ...
    st.info(
        "💡 **도메인 발견**: 카테고리의 자극 알고리즘 영역과 실제 발생 Fail_Type의 검토 영역이 "
        "7개 카테고리 전체에서 일치하지 않습니다. 영역 간 상호작용 검토 필요."
    )
...
def draw_model_drilldown(df, fail_df, target_model):
    """단일 모델 드릴다운 4-in-1"""
    plt.rcParams['font.family'] = font_name
    ...
    if len(target_fail) == 0:
        st.warning(f"{target_model} 모델은 Fail이 없습니다.")
        return

    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle(f'모델 드릴다운: {target_model}', fontsize=16, fontweight='bold')
    ...
    for i, row in fw_trend.iterrows():
        axes[0, 1].annotate(f"{row['count']}", (row['Build_Num'], row['count']),
                            textcoords="offset points", xytext=(0, 10),
                            ha='center', fontsize=10)
    axes[0, 1].set_title('빌드별 Fail 추이')
    axes[0, 1].set_xlabel('빌드 번호')
    axes[0, 1].set_ylabel('Fail 건수')
    ...
    axes[1, 0].set_xlabel('Fail율 (%)')

    # 우하: 메타 정보
    axes[1, 1].axis('off')
    total = len(target_data)
    fail = len(target_fail)
    fail_rate = fail / total * 100
    top_kw = kw_dist.idxmax()
    top_pct = kw_dist.max() / kw_dist.sum() * 100
    area = KEYWORD_DOMAIN_MAP.get(top_kw, {}).get('area', '?')
    priority = KEYWORD_DOMAIN_MAP.get(top_kw, {}).get('priority', '?')

    info_text = (
        f"[ 모델 정보 ]\n\n"
        f"전체 테스트: {total}건\n"
        f"Fail 건수: {fail}건 ({fail_rate:.1f}%)\n\n"
        f"[ 약점 진단 ]\n\n"
        f"우세 Fail_Type: {top_kw}\n"
        f"비중: {top_pct:.1f}%\n\n"
        f"검토 영역: {area}\n"
        f"우선순위: {priority}"
    )
    axes[1, 1].text(0.05, 0.95, info_text, fontsize=11,
...
# ============= 메뉴 2. Visualization =============
    ...
    st.markdown("### 3️⃣ 카테고리 × Fail_Type 히트맵 (도메인 발견)")
...
# ============= 메뉴 3. Analysis =============
    ...
    tab1, tab2, tab3, tab4 = st.tabs([
        "🔥 모델 히트맵", "🔍 모델 드릴다운", "🎯 우선순위 매트릭스", "📋 Fail 검색"
    ])
    ...
    with tab2:
        ...
        if selected_model:
            draw_model_drilldown(df, fail_df, selected_model)
    ...
    with tab4:
        ...
        with col3:
            model_filter = st.selectbox("Model 선택",
        ...
        display_df = filtered[['No', 'IC', 'Model', 'Keyword', 'Build_Num',
                                'Customer', 'Test_Item', 'video_id']].copy()
