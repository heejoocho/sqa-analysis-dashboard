...
if menu == "🏠 홈 (사이트 안내)":
    ...
    with col_a:
        st.markdown("""
        ### 🎯 필터 분석
        IC, 모델, 빌드, Test_Item, Fail_Type 등 **5가지 조건을 조합**해 원하는 데이터만 분석합니다. 
        회의 중 즉시 답변이 필요한 상황에 사용하세요.
        
        ### 📊 전체 인사이트
        전체 데이터의 핵심 패턴을 **6개 차트**로 한눈에 보여줍니다. 
        빌드별 추이, IC별 분포, 모델 히트맵, 카이제곱 통계 분석 등.
    ...
    with col_b:
        ...
        **모델별 다음 빌드 Fail율 예측**까지 한 화면에서.
        ...
        중복 검사 3단계로 데이터 무결성을 보장합니다.
        
        ### 📥 보고서 생성
        선택한 월의 데이터로 **회사 표준 PPT를 자동 생성**합니다. 
        Action Item까지 도메인 dict로 자동 매칭됩니다.
        """)
    ...
    st.success(
        "💡 **처음 사용하시나요?**\n\n"
        "왼쪽 사이드바에서 **'🎯 필터 분석'** 부터 시작해보세요. "
        "각 메뉴 상단의 'ℹ️ 사용법 안내' 박스를 클릭하면 자세한 사용법을 볼 수 있습니다.\n\n"
    ...
    show_menu_help(
        "🎯 필터 분석",
        "5가지 조건(IC/Model/Build/Test_Item/Fail_Type)을 조합해서 원하는 데이터만 골라 분석합니다.",
        [
            "각 필터에서 **여러 값을 동시에 선택** 가능합니다 (예: GT9XS와 GT1T0A 둘 다)",
            "필터를 **비워두면 전체 데이터**로 분석됩니다",
            "필터 조건에 따라 결과 KPI, 차트, 상세 표가 자동으로 갱신됩니다",
            "상세 표의 영상 링크를 클릭하면 결함 영상이 새 탭에서 재생됩니다",
        ]
    )
    st.markdown("---")
    ...
    show_menu_help(
        "📊 전체 인사이트",
        "전체 데이터의 핵심 패턴을 6개 차트로 보여줍니다. 큰 그림 → 세부 → 통계 검증 순서로 정렬되어 있습니다.",
        [
            "**1번 빌드별 ∩ 곡선**: 빌드가 진행되면서 Fail율이 어떻게 변하는지",
            "**2번 IC별 분포**: 어떤 IC가 어떤 결함에 약한지",
            "**3·4번 히트맵**: 카테고리/모델별 결함 패턴",
            "**5번 버블 차트**: 어떤 결함이 자주 발생 + 많은 모델에 영향 (오른쪽 위가 시급)",
            "**6번 카이제곱**: 합격률 vs 결함 종류 통계 검증 (대조 분석)",
        ]
    )
    st.markdown("---")
    ...
    show_menu_help(
        "📈 전체 통계",
        "시간·조직 관점에서 전체 데이터의 통계 정보를 보여줍니다.",
        [
            "**월별 추이**: 13개월간 테스트 건수와 Fail율 변화",
            "**고객사별 분석**: 어떤 고객사가 어떤 IC를 많이 쓰는지 (매트릭스)",
            "**Test_Item × Fail_Type**: 어떤 테스트 항목에서 어떤 결함이 잘 나오는지",
            "**데이터 메타 정보**: 수집 기간, IC 종류, 결과 분포 등",
        ]
    )
    st.markdown("---")
