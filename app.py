...
def render_data_upload():
    ...
    if uploaded_file is not None:
        try:
            ...
            # 데이터 추가 버튼
            st.markdown("---")
            if st.button("💾 데이터에 추가하기", type="primary", use_container_width=True):
    ...
    if st.session_state.uploaded_data:
        st.markdown("---")
        st.markdown("#### 📜 업로드 이력")
        for i, d in enumerate(st.session_state.uploaded_data, 1):
            st.markdown(f"**{i}. 추가된 데이터** — {len(d):,}건 (빌드: {sorted(d['Build_Num'].unique())})")
...
def render_report_generator(df, fail_df):
    ...
    st.caption("현재 데이터로 회사 표준 SQA Quality Review PPT를 자동 생성합니다.")
    
    # 보고서 설정
    col1, col2 = st.columns(2)
    with col1:
        report_date = st.date_input("📅 회의 일자", value=datetime.now())
    with col2:
        report_type = st.selectbox("📄 보고서 형식", ["SQA-SW Quality Review (10장)"])
    
    # 미리보기 정보
    st.markdown("---")
    st.markdown("#### 📋 생성될 보고서 정보")
    
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("📋 분석 데이터", f"{len(df):,}건")
    with col2:
        st.metric("❌ Fail 건수", f"{len(fail_df):,}건")
    with col3:
    ...
    if st.button("🚀 보고서 생성하기", type="primary", use_container_width=True):
        with st.spinner("📝 PPT 생성 중... (약 3~5초)"):
            try:
                ppt_buffer = generate_ppt_report(df, fail_df, datetime.combine(report_date, datetime.min.time()))
                st.success("✅ 보고서 생성 완료!")
                
                # 다운로드 버튼
                filename = f"SQA_Quality_Review_{report_date.strftime('%y%m%d')}.pptx"
                st.download_button(
                    label="📥 PPT 다운로드",
                    data=ppt_buffer,
                    file_name=filename,
...
def generate_ppt_report(df, fail_df, report_date):
    """PPT 보고서 생성 → BytesIO 반환"""
    prs = Presentation()
    ...
    add_text(s1, report_date.strftime("%Y. %m. %d"), 5, 5.5, 3.3, 0.5,
    ...
    info = [
        ...
        ("회의일자", report_date.strftime("%Y년 %m월 %d일")),
        ("참석자", "SQA 팀, SW 개발팀"),
    ...
    month_str = report_date.strftime("%Y년 %m월")
    add_text(s4, f"{month_str} Test Summary", 0.4, 1.4, 5, 0.4,
             size=14, bold=True, color=RPT_TITLE)
    
    item_summary = df['Test_Item'].value_counts().head(11).reset_index()
    item_summary.columns = ['Test_Item', 'count']
    rows_t1 = len(item_summary) + 2
    table1 = s4.shapes.add_table(rows_t1, 2, Inches(0.4), Inches(2),
                                  Inches(5.5), Inches(0.4 * rows_t1)).table
    table1.columns[0].width = Inches(3.5)
    table1.columns[1].width = Inches(2.0)
    set_cell(table1.cell(0, 0), "Test Item", bold=True, size=11,
             color=RPT_HEADER_FG, bg=RPT_HEADER_BG, align=PP_ALIGN.CENTER)
    set_cell(table1.cell(0, 1), month_str, bold=True, size=11,
             color=RPT_HEADER_FG, bg=RPT_HEADER_BG, align=PP_ALIGN.CENTER)
    total_count = 0
    for r, row in enumerate(item_summary.itertuples(), 1):
        bg = RPT_ALT_ROW if r % 2 == 0 else None
        set_cell(table1.cell(r, 0), row.Test_Item, size=10, bg=bg)
        set_cell(table1.cell(r, 1), row.count, size=10, bg=bg, align=PP_ALIGN.CENTER)
        total_count += row.count
    set_cell(table1.cell(rows_t1-1, 0), "Total", bold=True, size=11,
             bg=RPT_HEADER_BG, color=RPT_HEADER_FG, align=PP_ALIGN.CENTER)
    set_cell(table1.cell(rows_t1-1, 1), total_count, bold=True, size=11,
             bg=RPT_HEADER_BG, color=RPT_HEADER_FG, align=PP_ALIGN.CENTER)
    ...
    top_fail_kw = df[df['Result']=='FAIL']['Keyword'].value_counts().head(3)
    insight = f"• Top Fail_Type: {', '.join(top_fail_kw.index[:3])}\n"
    insight += f"• 영향 모델: {df[df['Result']=='FAIL']['Model'].nunique()}개\n"
    insight += f"• 영향 IC: {df[df['Result']=='FAIL']['IC'].nunique()}종\n"
    ...
    add_text(s5, "IC × 빌드별 Test 결과 상세", 0.4, 1.4, 10, 0.4,
             size=14, bold=True, color=RPT_TITLE)
    
    recent_builds = sorted(df['Build_Num'].unique())[-3:]
    recent_df = df[df['Build_Num'].isin(recent_builds)]
    summary = recent_df.groupby(['IC', 'Customer', 'Build_Num']).agg(
        total=('Result', 'count'),
    ...
    summary = summary.sort_values('fail_rate', ascending=False).head(10)
    
    if len(summary) > 0:
        rows = len(summary) + 1
        table = s5.shapes.add_table(rows, 7, Inches(0.4), Inches(2),
                                     Inches(12.6), Inches(0.4 * rows)).table
        widths = [1.0, 1.5, 2.5, 1.3, 1.5, 1.5, 1.3]
        for i, w in enumerate(widths):
            table.columns[i].width = Inches(w)
        headers = ['IC', '고객사', '빌드 번호', '총 Test', 'Pass', 'Fail', 'Fail율(%)']
        for c, h in enumerate(headers):
            set_cell(table.cell(0, c), h, bold=True, size=11,
                     color=RPT_HEADER_FG, bg=RPT_HEADER_BG, align=PP_ALIGN.CENTER)
        for r, row in enumerate(summary.itertuples(), 1):
            bg = RPT_ALT_ROW if r % 2 == 0 else None
            set_cell(table.cell(r, 0), row.IC, bold=True, size=10, bg=bg, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 1), row.Customer, size=10, bg=bg, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 2), f"R00.0.{row.Build_Num}", size=10, bg=bg, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 3), row.total, size=10, bg=bg, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 4), row.pass_n, size=10, bg=bg, align=PP_ALIGN.CENTER)
            fail_color = RPT_CRITICAL if row.fail_n > 0 else None
            set_cell(table.cell(r, 5), row.fail_n, size=10, bold=row.fail_n > 0, bg=bg, color=fail_color, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 6), f"{row.fail_rate}%", size=10, bold=row.fail_rate > 5, bg=bg,
                     color=fail_color if row.fail_rate > 5 else None, align=PP_ALIGN.CENTER)
    
    # ===== Slide 6: Fail Review =====
    s6 = prs.slides.add_slide(prs.slide_layouts[6])
    add_header(s6, 4, "Fail / Issue Review")
    add_text(s6, "주요 Fail 케이스 상세 (CRITICAL/HIGH 우선)", 0.4, 1.4, 10, 0.4,
             size=14, bold=True, color=RPT_TITLE)
    ...
    fail_df_copy = fail_df_copy.sort_values(['_sort', 'Build_Num'], ascending=[True, False]).head(8)
    
    if len(fail_df_copy) > 0:
        rows = len(fail_df_copy) + 1
        table = s6.shapes.add_table(rows, 6, Inches(0.4), Inches(2),
                                     Inches(12.6), Inches(0.45 * rows)).table
        ...
        for r, row in enumerate(fail_df_copy.itertuples(), 1):
            bg = RPT_ALT_ROW if r % 2 == 0 else None
            set_cell(table.cell(r, 0), r, size=10, bg=bg, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 1), row.Test_Item, size=9, bg=bg)
            set_cell(table.cell(r, 2), f"R00.0.{row.Build_Num}", size=9, bg=bg, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 3), f"{row.Model} / {row.IC}", size=9, bg=bg, align=PP_ALIGN.CENTER)
            priority = row.priority
            kw_color = RPT_CRITICAL if priority == 'CRITICAL' else (RPT_HIGH if priority == 'HIGH' else RPT_MID)
            set_cell(table.cell(r, 4), row.Keyword, size=9, bold=True, bg=bg, color=kw_color, align=PP_ALIGN.CENTER)
            desc = str(row.Fail_Description)[:100] if pd.notna(row.Fail_Description) else "-"
            set_cell(table.cell(r, 5), desc, size=8, bg=bg)
    
    # ===== Slide 7: Action Item (자동 생성) ⭐ =====
    ...
    if len(unique_actions) > 0:
        ...
        table = s7.shapes.add_table(rows, 6, Inches(0.4), Inches(2),
                                     Inches(12.6), Inches(0.5 * rows)).table
        ...
        for r, row in enumerate(unique_actions.itertuples(), 1):
            bg = RPT_ALT_ROW if r % 2 == 0 else None
            set_cell(table.cell(r, 0), r, size=10, bg=bg, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 1), f"R00.0.{row.Build_Num}", size=10, bg=bg, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 2), row.Model, size=10, bg=bg, align=PP_ALIGN.CENTER)
            priority = row.priority
            kw_color = RPT_CRITICAL if priority == 'CRITICAL' else (RPT_HIGH if priority == 'HIGH' else RPT_MID)
            set_cell(table.cell(r, 3), row.Keyword, size=10, bold=True, bg=bg, color=kw_color, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 4), priority, size=10, bold=True, bg=bg, color=kw_color, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 5), row.action + " 검토", size=10, bg=bg, bold=True)
    
    # ===== Slide 8: 회귀 알람 =====
    s8 = prs.slides.add_slide(prs.slide_layouts[6])
    add_header(s8, 6, "회귀 알람 (최신 빌드)")
    ...
    alerts = alerts.sort_values(['_sort', 'latest'], ascending=[True, False]).head(10)
    ...
    if len(alerts) > 0:
        ...
        for r, row in enumerate(alerts.itertuples(), 1):
            ...
            set_cell(table.cell(r, 0), row.상태, size=10, bold=True, color=state_color, bg=bg, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 1), row.Model, size=10, bg=bg, align=PP_ALIGN.CENTER)
            priority = row.priority
            kw_color = RPT_CRITICAL if priority == 'CRITICAL' else (RPT_HIGH if priority == 'HIGH' else RPT_MID)
            set_cell(table.cell(r, 2), row.Keyword, size=10, bold=True, color=kw_color, bg=bg, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 3), row.prev, size=10, bg=bg, align=PP_ALIGN.CENTER)
            set_cell(table.cell(r, 4), row.latest, size=10, bold=True, bg=bg, align=PP_ALIGN.CENTER)
            area = KEYWORD_DOMAIN_MAP.get(row.Keyword, {}).get('area', '?')
            set_cell(table.cell(r, 5), area, size=10, bg=bg)
    else:
    ...
    result_text = f"• 전체 Test {len(df):,}건 中 FAIL {len(fail_df):,}건 ({len(fail_df)/len(df)*100:.1f}%)\n"
    result_text += f"• 최신 빌드 R00.0.{latest_build}: FAIL {len(latest_fail)}건\n"
