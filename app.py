...
def render_filter_analysis(df, fail_df):
    ...
    # ===== 필터 UI =====
    st.markdown("### 🎯 필터 선택")
    st.caption("여러 조건을 조합해서 원하는 데이터만 분석할 수 있습니다. (회의 중 즉시 분석 가능)")
...
def render_full_insights(df, fail_df):
    """전체 데이터 핵심 인사이트 5개 차트"""
    plt.rcParams['font.family'] = font_name

    # ===== ① 빌드별 ∩ 곡선 =====
    st.markdown("### 1️⃣ 빌드별 ∩(역U) 안정화 곡선")
    st.caption("FW 빌드 진행에 따른 전체 Fail율 추이 (정점 후 안정화 진행)")
    ...
    st.caption("IC마다 우세 Fail_Type이 완전히 다름 → IC별 맞춤 검토 필요")
    ...
    st.caption("카테고리의 자극 영역과 실제 발생 Fail_Type 영역의 불일치 발견")
    ...
    st.info("💡 **도메인 발견**: 7개 카테고리 모두에서 자극 알고리즘 영역과 실제 발생 Fail_Type 영역이 일치하지 않습니다.")

    st.markdown("---")

    # ===== ④ 모델 × Fail_Type 히트맵 =====
    st.markdown("### 4️⃣ 모델 × Fail_Type 히트맵 ⭐ 시그니처 차트")
    st.caption("46개 모델 × 8개 Fail_Type 매트릭스로 모델별 약점 한눈에")
    ...
    st.markdown("### 5️⃣ Severity × Priority 우선순위 매트릭스")
    st.caption("결함별 심각도와 우선순위를 한눈에 — 무엇부터 잡을지 결정")
...
st.sidebar.markdown("""
- **🎯 필터 분석** (메인)  
  IC/모델/빌드/Fail_Type 조합 필터 → 동적 분석
- **📊 전체 인사이트**  
  전체 데이터 핵심 차트 5개
- **🚨 회귀 알람**  
